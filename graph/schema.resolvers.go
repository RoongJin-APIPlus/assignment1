...
func (r *mutationResolver) AddPokemon(ctx context.Context, input model.NewPokemon) (*model.Pokemon, error) {
	...
	for i := range input.Type {
		typeString += input.Type[i]
	}
	for i := range input.Abilities {
		abilitiesString += input.Abilities[i]
	}

	var id int64
	id, err := r.DB.AddPokemon(input.Name, input.Description, input.Category, typeString, abilitiesString)
...
func (r *mutationResolver) UpdatePokemon(ctx context.Context, input model.NewPokemon) (*model.Pokemon, error) {
	...
	r.DB.UpdatePokemon(int(id_int64), input.Name, input.Description, input.Category, typeString, abilitiesString)
